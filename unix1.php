...
if($_SERVER["REQUEST_METHOD"]==="POST"){
    header("Location:/unix1.php");
}
try{
    $pdo = new PDO("mysql:hpst=localhost;
    dbname=zemi1;charset=utf8","root","",
...
if($item_name && isset($_POST["btn"])){
    ...
    $stmt = $pdo -> prepare("INSERT INTO hyouka(name,point) VALUES (:name,:point)");
    $stmt->bindParam(':name',$item_name,PDO::PARAM_STR);
    $stmt->bindValue(':point',$item_point,PDO::PARAM_INT);
    $stmt->execute();
    }
    $select_sql = "SELECT*FROM hyouka";
...
<body>
    <form action="unix1.php" method="post">
        <div class="header">
            <h1>評価.com</h1>
        </div>
        <div class="container">
            ...
            <div class="output">
                ...
                <p><?php echo $item_evaluation[0] . ':' . $item_evaliation[1] . '点'; ?>
                <?php endforeach;?>
            </div>   
        </div>
        <div class="footer">
